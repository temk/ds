...
filter_str::filter_str(const error_handler &ref, type_t t_in, type_t t_out, size_t width, size_t cap, const string &key, driver * drv, bool read_dict)
  : filter(ref, t_in, size_of(t_in), t_out, size_of(t_out), width), cap_(cap), driver_(drv) {

  lookup_ = lookup::create(ref, key, t_in, t_out);
  if (read_dict) {
    driver_ ->read_dictionary(*lookup_);
  }
  buff_ = new char[size_out() * cap_* width];
}
...
filter_str::put(const void *data, size_t num) {
  ...
  while(num > 0) {
    ...
    size_t w = width();
    lookup_ ->lookup_index(data, buff_, adv, chunk * width());
...
filter_str::get(size_t offs, size_t num, void *data) {
  ...
  while(num > 0) {
    ...
    lookup_ ->lookup_value(buff_, data, adv, chunk * width());
    num -= chunk;
    adv += chunk * width();
    offs += chunk;
  }
